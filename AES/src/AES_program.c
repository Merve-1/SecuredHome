...
// AES inverse S-box array values for decryption
...
static u8 round_keys[AES_BLOCK_SIZE * 11];
...
static void SubBytes(u8 *state, const u8 *sbox_table) {
...
static void AddRoundKey(u8 *state, const u8 *round_key) {
...
void AES_decrypt(const u8 *input, u8 *output) {
	...
	for (int round = 9; round > 0; --round) {
		...
		AddRoundKey(state, round_keys + round * AES_BLOCK_SIZE);
		MixColumns(state); // For decryption, MixColumns is applied after AddRoundKey
	}
